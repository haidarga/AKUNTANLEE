...
export default function WorkpaperClient({
  ...
  initialWorkpaper?: WorkpaperVersion | null;
  initialLines?: WorkpaperLineItem[];
  initialChecks?: ValidationCheckResult[];
}) {
  ...
  const activeWp = initialWorkpaper || state.workpaperVersions.find((w) => w.engagementId === engagement.id) || (isCustomEngagement ? emptyWp : state.workpaperVersions[0]);
  const [wpVersion, setWpVersion] = useState<WorkpaperVersion>(activeWp);
  const startingLines = initialLines.length > 0 ? initialLines : (isCustomEngagement ? [] : state.workpaperLines);
  const startingChecks = initialChecks.length > 0 ? initialChecks : (isCustomEngagement ? [] : state.validationChecks);
  const [lines, setLines] = useState<WorkpaperLineItem[]>(startingLines);
  const [checks, setChecks] = useState<ValidationCheckResult[]>(startingChecks);
