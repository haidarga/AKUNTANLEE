...
export default function FilesClient({
  ...
  const activeFiles = initialFiles.length > 0
    ? initialFiles
    : (engagementId === 'ENG-2026-01' ? state.fileVersions.filter((f) => f.engagementId === engagement.id) : []);
  const initialAccCount = initialAccounts.length > 0
    ? initialAccounts.length
    : (activeFiles.length > 0 ? (engagementId === 'ENG-2026-01' ? 22 : null) : null);
  const [fileVersions, setFileVersions] = useState<FileVersion[]>(activeFiles);
  const [extractedCount, setExtractedCount] = useState<number | null>(initialAccCount);
  ...
  useEffect(() => {
    fetch('/api/v1/engagements/' + engagement.id + '/files', { credentials: 'include' })
      .then((res) => res.json())
      .then((json) => {
          ...
          {fileVersions.map((fv) => (
            <div key={fv.id} className="p-4 hover:bg-[#F6F7F5]/50 transition-colors flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div className="space-y-1.5 flex-1">
                ...
                <div className="text-[11px] text-[#52636A]">
                  Daftar Sheet: <span className="font-medium text-[#102A32]">{(fv.sheetNames || ["Sheet1"]).join(', ')}</span>
                </div>
              </div>
